...
def save_product(store_id: str, sku: str, name: str, supplier: str,
                 ...
                 unit: str = "un", sale_price: str = "0", opening_balance: int | None = None) -> bool:
    ...
    with session() as db:
        ...
        db.execute("""INSERT INTO products(store_id,sku,name,category,unit,sale_cents,supplier,
                    ...
                    sale_cents=excluded.sale_cents,supplier=excluded.supplier,
                   ...
                   (store_id, sku, name, category, unit, cents, supplier,
                    lead_days, review_days, safety, minimum, multiple))
        if not existed and opening_balance is not None:
            add_in(db, store_id, sku, "Saldo inicial", opening_balance, "opening:"+uuid4().hex)
    return not existed
